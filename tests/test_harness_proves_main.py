...
Until 2026-09-18 it fired `on: [push, pull_request]`, so every harness branch
cost two identical runs of the same tree — 76% of the 24,500 runs September
billed — and the branch-push half never did anything: the gate wakes only on
`workflow_run.event == 'pull_request'` (bureau-pipeline merge-gate.yml). These
tests hold the file at the scaffold's shape, and hold the runner where it is,
so either moves only by a change that fails here first.
"""
...
def test_the_runner_choice_is_explained_where_it_is_made():
    ...
    # reason: the job count, and the mini queue it would otherwise drown.
    above = [l.strip() for l in head.splitlines()[-12:] if l.strip().startswith("#")]
    rationale = " ".join(above).lower()
    assert "minis" in rationale and "queue" in rationale and "a day" in rationale, (
        "the comment above runs-on must say why the sandbox stays on "
        "ubuntu-latest — the per-day job count and the mini queue"
    )
